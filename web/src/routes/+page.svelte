<svelte:head>
	<title>Blaadje — wedstrijdapp voor jeugdtrainers</title>
	<meta
		name="description"
		content="Je opstelling, je wissels en de speeltijd, bijgehouden terwijl je coacht. Werkt zonder bereik en zonder account, gewoon op je telefoon."
	/>
</svelte:head>

<div class="landing">
	<header class="kop">
		<span class="merk">Blaadje</span>
		<a class="knop prim" href="/app">Openen</a>
	</header>

	<section class="hero">
		<h1>Het blaadje in je hand,<br />maar dan op je telefoon</h1>
		<p class="groot">
			Je opstelling, je wissels en de speeltijd, bijgehouden terwijl je coacht. Een wissel kost twee tikken, en na
			afloop weet je precies wie hoe lang heeft gespeeld.
		</p>
		<div class="knoprij">
			<a class="knop prim groot" href="/app">Blaadje openen</a>
			<span class="klein">Gratis · geen account nodig · werkt zonder bereik</span>
		</div>
	</section>

	<section class="plaat">
		<img
			src="/scherm-wedstrijd.png"
			width="390"
			height="700"
			alt="Het wedstrijdscherm met de opstelling op een veld en de bank ernaast"
		/>
	</section>

	<section class="blokken">
		<div>
			<h2>Wisselen in twee tikken</h2>
			<p>
				Tik wie eruit gaat, tik wie erin komt. De bank staat naast het veld, in dezelfde volgorde als de linies, en
				bovenaan staat wie tot nu toe het minst heeft gespeeld. Twee spelers van plek laten ruilen kan ook, zonder dat
				er iemand van de bank hoeft te komen.
			</p>
		</div>
		<div>
			<h2>Speeltijd zonder invoeren</h2>
			<p>
				De minuten rollen uit je wissels, dus je hoeft niets bij te houden. Onder elke naam op het veld zie je hoe lang
				die er al staat. Minuten in het doel tellen apart, want een helft keepen is geen halve wedstrijd voetballen.
			</p>
		</div>
		<div>
			<h2>Helften of kwarten</h2>
			<p>
				Van 11 tegen 11 tot 4 tegen 4, in twee helften of in vier kwarten. Nergens zit een vast aantal spelers
				ingebakken, dus voor de kleinsten werkt het net zo goed als voor de grote elftallen.
			</p>
		</div>
		<div>
			<h2>Presentie op de training</h2>
			<p>
				Aanwezig, afgemeld of niet gekomen: één tik per speler. Wie de laatste keren weinig kwam, zie je terug op het
				moment dat je je opstelling maakt. De app zet het er alleen bij; wat je ermee doet is aan jou.
			</p>
		</div>
		<div>
			<h2>Een verslagje voor de groepsapp</h2>
			<p>
				Na afloop ligt er een kant-en-klaar bericht: de uitslag, wie er scoorden, en de paar regels die je er zelf bij
				schrijft. De wissels blijven eruit, want daar hoeven de ouders niets van te vinden.
			</p>
		</div>
		<div>
			<h2>Langs de lijn, niet achter een bureau</h2>
			<p>
				Het scherm blijft aan zolang de klok loopt, scrollen hoeft nergens, en zonder bereik gaat alles gewoon door.
				Wat je invult blijft op je eigen telefoon staan.
			</p>
		</div>
	</section>

	<section class="plaat smal">
		<img
			src="/scherm-spelers.png"
			width="390"
			height="700"
			loading="lazy"
			alt="Het spelersoverzicht met speeltijd en presentie per speler"
		/>
		<p class="onderschrift">
			Na een paar weken zie je in één lijst wie hoeveel heeft gespeeld en hoe vaak die op de training stond.
		</p>
	</section>

	<section class="rustig">
		<h2>Waar je gegevens blijven</h2>
		<p>
			Alles staat op je eigen toestel. Inloggen hoeft niet, en zolang jij dat niet wilt gaat er niets naar een server.
			Wil je je laptop en je telefoon gelijk houden, dan kan dat met een account. Je gegevens gaan dan over een
			beveiligde verbinding, en jij bent de enige die bij je eigen team kan.
		</p>
		<p>
			Namen van kinderen zijn geen bijzaak. Daarom bewaart Blaadje alleen wat het echt nodig heeft: een voornaam, een
			linie, en de minuten die uit je wissels volgen. Geen beoordelingen, geen dossier.
		</p>
	</section>

	<section class="rustig">
		<h2>Waar het vandaan komt</h2>
		<p>
			Blaadje is gemaakt door een ouder-trainer voor zijn eigen O14. Alles wat er al was bleek een tactiekbord, terwijl
			je langs de lijn juist een wisselschriftje nodig hebt. Het wordt elke week gebruikt bij een echte wedstrijd, en
			dat verklaart meteen waarom het doet wat het doet, en niet meer dan dat.
		</p>
		<p>
			Het is nog volop in ontwikkeling en gratis te gebruiken. Loop je ergens tegenaan of mis je iets, laat het vooral
			weten.
		</p>
		<div class="knoprij">
			<a class="knop prim groot" href="/app">Aan de slag</a>
		</div>
	</section>

	<footer>
		<p>Blaadje · gemaakt in Nederland · <a href="/app">naar de app</a></p>
	</footer>
</div>
