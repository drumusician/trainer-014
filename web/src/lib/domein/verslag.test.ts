...
import { timelineRows, eventText, reportText } from './verslag';
...
describe('verslag', () => {
	it('laat de wissels weg en zet de stand op volgorde van de klok', () => {
		const tekst = reportText(bron, spelers);
	...
	it('zet de wissels erbij als je dat wilt', () => {
		const tekst = reportText(bron, spelers, true);
	...
	it('draait de stand om bij een uitwedstrijd', () => {
		expect(reportText({ ...bron, thuis: false }, spelers)).toContain('Ajax – JO11-2 1–2');
	});

	it('valt terug op een neutrale naam als er geen teamnaam is', () => {
		expect(reportText({ ...bron, teamnaam: undefined }, spelers)).toContain('Ons team – Ajax');
		expect(reportText({ ...bron, teamnaam: '  ' }, spelers)).toContain('Ons team – Ajax');
	});
});
...
describe('kwarten en een notitie', () => {
	it('benoemt de pauzes naar het deel', () => {
		...
		expect(eventText(g, spelers, undefined, 4)).toBe('Pauze — 1e kwart voorbij');
		expect(eventText({ ...g, deel: 2 }, spelers, undefined, 4)).toBe('Rust — 2e kwart voorbij');
		expect(eventText({ ...g, deel: 1 }, spelers, undefined, 2)).toBe('Rust — 1e helft voorbij');
	});

	it('zet de notitie onderaan het verslag', () => {
		const tekst = reportText({ ...bron, notitie: '  Sterk begin.  ' }, spelers);
		expect(tekst.endsWith('\n\nSterk begin.')).toBe(true);
	});

	it('laat het verslag met rust als er niets geschreven is', () => {
		expect(reportText({ ...bron, notitie: '   ' }, spelers).endsWith('doelpunt')).toBe(true);
	});
});
...
describe('van plek ruilen in het verloop', () => {
	...
	it('noemt wie waar naartoe ging', () => {
		...
		expect(eventText(g, spelers, undefined, 2, '4-3-3')).toBe('Aad naar SP, Bram naar K');
	});

	it('gebruikt de leesbare naam van de plek', () => {
		const g = { type: 'ruil' as const, t: 800, plekA: 'CVl', plekB: 'TIEN', spelerA: 'p1', spelerB: 'p2' };
		expect(eventText(g, spelers, undefined, 2, '4-4-2 ruit')).toBe('Aad naar 10, Bram naar CV');
	});

	it('houdt oude wedstrijden zonder namen leesbaar', () => {
		const g = { type: 'ruil' as const, t: 800, plekA: 'K', plekB: 'SP' };
		expect(eventText(g, spelers, undefined, 2, '4-3-3')).toBe('Van plek gewisseld: K en SP');
	});
});
...
describe('een wissel in het verloop', () => {
	it('zegt er ook bij op welke plek', () => {
		...
		expect(eventText(g, spelers, undefined, 2, '4-4-2 ruit')).toBe('Bram voor Aad op 10');
	});
});
...
describe('ruilen op hetzelfde moment samenvatten', () => {
	...
	it('maakt van een rondje van vier één regel met de netto verhuizing', () => {
		...
		const regels = timelineRows(g, vier, undefined, 2, '4-4-2 ruit');
	...
	it('laat ruilen op verschillende tijdstippen apart staan', () => {
		...
		expect(timelineRows(g, spelers, undefined, 2, '4-3-3')).toHaveLength(2);
	});
	...
	it('houdt de plek in de lijst kloppend, zodat een doelpunt te wissen blijft', () => {
		...
		const regels = timelineRows(g, spelers, undefined, 2, '4-3-3');
